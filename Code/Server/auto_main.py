        ...
        try:
            ...
            if center:
                ...
                if radius > 15:  # Significant object size (adjust for ball)
                    ...
                    if d < 10:    # Too close (negative PID indicates error below target)
                        self.move("backward")  
                    elif d > 13: # Too far (positive PID indicates error above target)
                        self.move("forward")  
                    ...
                    self.stop() # Stop if object too small
            else:
                self.stop()     # Stop if no valid center
